...
- **Chart stub (DEV):** common Klarbog codes — bank `1000`, AR `1500`, AP `4400`, expense band `4000`–`6999` (bank-CSV often uses `5800` / income `6100` inside that band). Empty or non-digit account → hard fail `dk.bookkeeping.account_range`. Digit account outside the stub → optional hint `dk.bookkeeping.known_account` (does **not** block). Read-only list: HTTP `GET /api/v1/rules/chart?company=…` and MCP `rules_chart_list` (same stub codes + labels; AuthZ/allowlist; **no** journal write).
...
See dedicated skill [`moms-suggest.md`](moms-suggest.md) and **ADR-011**.
When building legs and the memo will carry `#vat25`, call that surface **before**
preview — suggested net+vat i64 amounts; **never** auto-posts (`auto_post: false`).
