...
pub fn registered_rules() -> &'static [RegisteredRule] {
    &[
        ...
        RegisteredRule {
            ...
            provisions: &["§ 5, stk. 1"],
            severity: "hard_stop",
            enforced_by: "invoice-plugin reminders: max 100 DKK; max 3 per claim; min 10 dage mellem; overdue + positivt inddriveligt hovedstol; DKK only; sent/part_paid; dual-write SQLite invoice_reminders + audit_log invoice_reminder_register",
            proven_by: &[
                "klarbog-plugin-invoice reminders_tests::registers_statutory_reminder_fee_on_overdue_invoice",
                ...
                "klarbog-plugin-invoice claim_ledger_tests::reminder_register_writes_sqlite_row_and_audit",
                "klarbog-plugin-invoice claim_ledger_tests::reminder_re_register_failure_does_not_double_sqlite",
                "klarbog-store-sqlite claim_audit::tests::reminder_insert_then_duplicate_is_idempotent",
            ],
            gaps: &[
        ...
        RegisteredRule {
            ...
            provisions: &["§ 5, stk. 1"],
            severity: "hard_stop",
            enforced_by: "invoice-plugin: register_late_interest persisterer immutable claim i invoices.json; dual-write SQLite invoice_interest_claims + audit_log invoice_interest_register; duplikat (dato+rate) afvist; claim_open_balance_minor",
            proven_by: &[
                "klarbog-plugin-invoice late_interest_tests::register_rejects_duplicate_and_zero_increment",
                "klarbog-plugin-invoice late_interest_tests::staged_claims_bill_incrementally",
                "klarbog-plugin-invoice claim_ledger_tests::interest_register_writes_sqlite_and_rejects_dup_without_double",
                "klarbog-store-sqlite claim_audit::tests::interest_and_compensation_idempotent_uniques",
        ...
        RegisteredRule {
            ...
            provisions: &["§ 9a, stk. 1"],
            severity: "hard_stop",
            enforced_by: "invoice-plugin: register_invoice_compensation persisterer immutable claim i invoices.json; dual-write SQLite invoice_compensation_claims + audit_log invoice_compensation_register; duplikat afvist; claim_open_balance_minor inkl. total_compensation_minor",
            proven_by: &[
                "klarbog-plugin-invoice late_compensation_tests::register_rejects_duplicate_and_private_buyer",
                "klarbog-plugin-invoice late_compensation_tests::claim_open_balance_includes_compensation",
                "klarbog-plugin-invoice claim_ledger_tests::compensation_register_writes_sqlite_and_rejects_dup_without_double",
                "klarbog-store-sqlite claim_audit::tests::interest_and_compensation_idempotent_uniques",
        ...
        RegisteredRule {
            ...
            severity: "hard_stop",
            enforced_by: "invoice-plugin: fortløbende {scope}-{NNNN}; immutable JSON+PDF under invoices/issued/ with top-level currency; fx_rate_to_dkk on draft create (UI/MCP/HTTP) + DKK totals on issue; sha256 + pdf_sha256 in documents.json notes; retain_until",
            proven_by: &[
                "klarbog-plugin-invoice invoice_numbers::tests",
                "klarbog-plugin-invoice fx::tests::converts_eur_totals_at_rate",
                "klarbog-plugin-invoice fx_parse::tests::parses_common_rates",
                "klarbog-plugin-invoice store::tests::eur_draft_stores_fx_rate",
                ...
                "klarbog-mcp invoice::tests::mcp_invoice_create_eur_requires_fx_rate",
